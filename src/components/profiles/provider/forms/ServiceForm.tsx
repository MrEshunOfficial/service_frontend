...
interface ServiceFormProps {
  ...
  onCancel?: () => void;
  embedded?: boolean; // New prop to indicate if form is embedded in a dialog
}
...
export default function ServiceForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Only redirect if not embedded (i.e., standalone page)
      if (!embedded) {
        setTimeout(() => {
      ...
      toast.error(
        `Failed to ${mode} service: ${err instanceof Error ? err.message : "Unknown error"}`,
      );
    }
  };
  ...
  return (
    <div className="w-full">
      ...
      >
        ...
        <ScrollArea
          className={cn(
            embedded ? "h-[calc(90vh-200px)]" : "h-[calc(92vh-280px)]",
          )}
        >
